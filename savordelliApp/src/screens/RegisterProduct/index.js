  ...
  export function RegisterProduct({ }) {
    ...
    const handleCameraImagePicker = async () => {
      ...
      if (canceled) {
        ...
        uploadImageCloudinary(assets[0].uri)
  
      }
    };
  
    //mandando a imagem escolhida para o server da cloudnary
    const uploadImageCloudinary = async (assets) => {
      ...
      uploadData.append('upload_preset', 'jbieku8i');
      uploadData.append("cloud_name", "donkjmht1")
  
      console.log(uploadData)
  
      try {
        const response = await fetch("https://api.cloudinary.com/v1_1/donkjmht1/upload", {
          method: 'POST',
          headers: {
    ...
    //   const kk = async (formData) =>{
  
    //     const uploadData = new FormData(); 
    //     uploadData.append('file', formData)
    //     uploadData.append('upload_preset', 'jbieku8i');
    //     const response = await axios.post('https://api.cloudinary.com/v1_1/donkjmht1/image/upload', formData, {
    //   headers: {
    //     'Content-Type': 'multipart/form-data',
    ...
    // const cloudinaryUpload = async (source) => {
  
  
    //   const data = new FormData();
    //   data.append('file', source)
    //   data.append('upload_preset', 'jbieku8i')
    //   data.append("cloud_name", "donkjmht1")
    //   console.log(data)
    //   try {
    //     const response = await fetch("https://api.cloudinary.com/v1_1/donkjmht1", {
    //       method: "POST",
