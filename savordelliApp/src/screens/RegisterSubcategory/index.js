...
export function RegisterSubcategory({ }) {
    ...
    const schema = yup.object({
        subcategoria: yup.string().max(20, "Nome muito grande").required("Informe a subcategoria desejada"),
    ...
    async function handleRegisterSubcategoria(data) {
        try {
            ...
            console.log(data)
            if (response.ok) {
                ToastAndroid.show('Subcategoria cadastrada com sucesso 🎉!', ToastAndroid.SHORT);

            } else {
                console.error('Erro ao cadastrar a subcategoria:', response);
                        ...
                        <View style={styles.containerInputs}>
                            ...
                            <Controller
                                ...
                                render={({ field: { onChange, onBlur, value } }) => (
                                    <TextInput
                                        style={[styles.input, {
                                            borderWidth: errors.subcategoria && 2,
                                            borderColor: errors.subcategoria && '#ff375b'
                                        }]}
                                        placeholder={"Digite o novo nome da sua nova subcategoria"}
                            ...
                            {errors.subcategoria && <Text style={styles.labelError}> {errors.subcategoria?.message} </Text>}

                            <View style={styles.containerButtons}>

                                <TouchableOpacity onPress={handleSubmit(handleRegisterSubcategoria)} style={styles.buttonCadastro}>
